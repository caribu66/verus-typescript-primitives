...
import { fromBase58Check } from "./address";
...
export const IDENTITY_OFFER_BASE_KEY = "04e9923a4e0736bdf2098564cb3e7c68f9d12359";
// hash160result of vrsc::system.exchange.currencyoffer
export const CURRENCY_OFFER_BASE_KEY = "37ea50baa4f2fb07e33252be3202319e35790380";
// hash160result of vrsc::system.exchange.offerforidentity
export const OFFER_FOR_IDENTITY_BASE_KEY = "1a077aae5e356da8191e780c8a68452998a86201";
// hash160result of vrsc::system.exchange.offerforcurrency
export const OFFER_FOR_CURRENCY_BASE_KEY = "ecbb3a6edc834d4dbdc7fb83514cf65e97667eba";

const sha256 = (b: Buffer): Buffer => createHash("sha256").update(b).digest();
const dsha256 = (b: Buffer): Buffer => sha256(sha256(b));
 ...
 */
export const deriveOfferIndexKey = (
  baseKeyHash160Hex: string,
  targetIAddress: string
): Buffer => {
  const target = fromBase58Check(targetIAddress).hash;
 ...
 */
export const buildListingDepositScript = (
  forIndexKey: Buffer,
  offerIndexKey: Buffer,
  ownerHash160: Buffer
): Buffer => {
...
export const buildListingOpReturnScript = (signedOfferTxHex: string): Buffer => {
  ...
  let push: Buffer;
  if (payload.length <= 75) push = Buffer.from([payload.length]);
  else if (payload.length <= 0xff) push = Buffer.from([0x4c, payload.length]);
  else {
    push = Buffer.alloc(3);
    push[0] = 0x4d;
